...
final class M4ReportsAcceptanceUITests: XCTestCase {
    ...
    func testUS7DashboardUsesEveryRangeAndShowsHonestObservedEvidence() {
        ...
        requireByScrolling(
            chart("reports.chart.body.kg.segment.1", in: app),
            in: app,
            "The shipped dashboard must chart deterministic weight observations."
        )
        requireByScrolling(
            chart("reports.chart.body.cm.segment.1", in: app),
            in: app,
        ...
        XCTAssertTrue(proteinEvidence.label.contains("1/2 uygun hedef günü"))
        XCTAssertTrue(proteinEvidence.label.contains("hedefsiz 1 gün dışlandı"))
        requireByScrolling(
            app.staticTexts.matching(
        ...
        requireByScrolling(chart("reports.chart.sleep.segment.1", in: app), in: app)
        requireByScrolling(chart("reports.chart.mood.segment.1", in: app), in: app)
        requireByScrolling(chart("reports.chart.posture.segment.1", in: app), in: app)
        requireByScrolling(
            app.staticTexts.matching(
        ...
        let rangeEvidence: [(String, Int, [String], [String])] = [
            ("reports.range.one_month", 2, ["79,4 kg", "79 kg"], ["81 kg", "82 kg", "84,5 kg"]),
            ("reports.range.three_months", 3, ["81 kg", "79,4 kg", "79 kg"], ["82 kg", "84,5 kg"]),
            ("reports.range.six_months", 4, ["82 kg", "81 kg", "79,4 kg", "79 kg"], ["84,5 kg"]),
            ("reports.range.one_year", 5, ["84,5 kg", "82 kg", "81 kg", "79,4 kg", "79 kg"], []),
        ]
        for (identifier, expectedCount, includedValues, excludedValues) in rangeEvidence {
            ...
            assertWeightTable(
                ...
                includes: includedValues,
                excludes: excludedValues,
                in: app
            )
        }
        ...
        requireByScrolling(
            app.staticTexts.matching(NSPredicate(format: "label CONTAINS %@", "1TM"))
                .firstMatch,
            in: app
        )
    ...
    func testCSVJSONAndBothExportsUseRealShareAndCleanEveryArtifact() {
        ...
        for (formatIndex, includesPhotos) in [(0, false), (1, false), (2, true)] {
            ...
            require(
                ...
                "The real system activity sheet must expose its explicit close action."
            ).tap()
    ...
    func testExportFailureKeepsSelectionAndRetryThenDismissalCleansArtifact() {
        ...
        let firstGenerate = require(app.buttons["reports.export.generate"])
        makeHittable(firstGenerate, in: app)
        firstGenerate.tap()
        requireByScrolling(
            identified("reports.export.error", in: app),
            in: app,
            "The deterministic fail-once export must publish its recoverable error."
        )
        XCTAssertEqual(
            requireByScrolling(app.switches["reports.export.photos"], in: app).value as? String,
            "1"
        )
        assertWorkspaceCountEventually(0, in: app)

        requireByScrolling(app.buttons["reports.export.retry"], in: app).tap()
        requireByScrolling(
            app.buttons["reports.export.share"],
            in: app,
            "Retry must produce a shareable export through the shipped coordinator."
        )
        XCTAssertEqual(
            requireByScrolling(app.switches["reports.export.photos"], in: app).value as? String,
            "1"
    ...
    func testSlowExportCanCancelAndRetryWithoutLeavingTemporaryFiles() {
        ...
        firstGenerate.tap()
        requireByScrolling(
            identified("reports.export.progress", in: app),
            in: app,
            "The deterministic slow export must expose real progress."
        )
        requireByScrolling(app.buttons["reports.export.cancel"], in: app).tap()
        let retryGenerate = requireByScrolling(app.buttons["reports.export.generate"], in: app)
        assertWorkspaceCountEventually(0, in: app)
        ...
        requireByScrolling(
            app.buttons["reports.export.share"],
            in: app,
            "Retry after cancellation must produce a shareable export."
        )
        assertWorkspaceCountEventually(1, in: app)
        dismissExportSheet(in: app)
        assertWorkspaceCountEventually(0, in: app)
    ...
    @discardableResult
    private func optIntoPhotos(in app: XCUIApplication) -> XCUIElement {
        let photos = requireByScrolling(app.switches["reports.export.photos"], in: app)
        let initialValue = photos.value as? String
        XCTAssertEqual(initialValue, "0", "Photo binaries must remain opt-in by default.")
        photos.coordinate(withNormalizedOffset: CGVector(dx: 0.9, dy: 0.5)).tap()

        let refreshed = requireByScrolling(app.switches["reports.export.photos"], in: app)
        waitUntilValue("1", of: refreshed)
        XCTAssertEqual(refreshed.value as? String, "1")
        return refreshed
    }

    private func assertWeightTable(
        expectedCount: Int,
        includes includedValues: [String],
        excludes excludedValues: [String],
        in app: XCUIApplication
    ) {
        let table = requireByScrolling(
            identified("reports.table.body.kg.segment.1", in: app),
            in: app,
            "Every selected range must expose its visible weight evidence table."
        )
        let weightRows = table.descendants(matching: .any).matching(
            NSPredicate(format: "label CONTAINS %@", " kg")
        )
        if weightRows.count == 0 {
            table.tap()
        }
        require(
            table.descendants(matching: .any).matching(
                NSPredicate(format: "label CONTAINS %@", "79 kg")
            ).firstMatch,
            "The expanded weight table must publish its current observation before range assertions.",
            timeout: 5
        )
        XCTAssertEqual(
            weightRows.count,
            expectedCount,
            "The selected range must expose exactly its observed weight rows."
        )
        for value in includedValues {
            XCTAssertEqual(
                ...
                1,
                "The selected range must include visible weight evidence \(value)."
            )
        }
        for value in excludedValues {
            XCTAssertEqual(
                ...
                0,
                "The selected range must exclude out-of-range weight evidence \(value)."
            )
        }
    }
    ...
    private func waitUntilValue(_ value: String, of element: XCUIElement) {
        let expectation = XCTNSPredicateExpectation(
    ...
    private func requireByScrolling(
        ...
        M4UITestScrollHelper.requireByScrolling(
            in: app,
            query: { element },
    ...
    private func chart(_ identifier: String, in app: XCUIApplication) -> XCUIElement {
