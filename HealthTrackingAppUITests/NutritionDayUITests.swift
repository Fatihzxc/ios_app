...
final class NutritionDayUITests: XCTestCase {
    ...
    func testDarkAX5ReduceMotionAndIncreaseContrastProduceCanonicalEvidence() throws {
        ...
        for identifier in [
            ...
            XCTAssertTrue(
                isContainedWithPixelRounding(element.frame, in: ax5),
                "AX5 must wrap vertically without clipping \(identifier). "
                    + "App frame: \(ax5.frame); element frame: \(element.frame)."
            )
        }
    ...
    private func assertFiftyTwoPointTarget(
        ...
        XCTAssertGreaterThanOrEqual(element.frame.width + 0.01, 52)
    ...
    private func makeVisible(_ element: XCUIElement, in app: XCUIApplication) {
        for _ in 0..<10 {
            ...
            if !frame.isEmpty, isContainedWithPixelRounding(frame, in: app) { return }
    ...
    private func isContainedWithPixelRounding(
        _ frame: CGRect,
        in app: XCUIApplication
    ) -> Bool {
