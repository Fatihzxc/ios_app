...
TASK9_FULL_JOB_TIMEOUT = "360"
TASK9_FULL_SUITE_TIMEOUT = "150"
...
TASK9_STAGE_A_SWIFT_PATHS = (
    ...
    "HealthTrackingAppUITests/M4UITestScrollHelper.swift",
...
def verify_task9_stage_a_contract(root: Path) -> None:
    ...
    full_suite_step = (
        ...
        "        run: scripts/test-ios.sh --skip-testing "
        "HealthTrackingAppUITests/TodayGuidanceUITests/"
        "testColdLaunchPublishesFirstMeaningfulDirectiveWithinOneSecondMedian\n"
    )
    if full_job.count(full_suite_step) != 1:
        raise ValueError(
            "M4.9 complete functional suite must retain its exact 150-minute fail-closed budget"
        )
    ...
    for step_name, timeout, selector in (
        (TASK9_ACCEPTANCE_STEP_NAME, TASK9_ACCEPTANCE_TIMEOUT, TASK9_ACCEPTANCE_SELECTOR),
        (TASK9_ACCESSIBILITY_STEP_NAME, 60, TASK9_ACCESSIBILITY_SELECTOR),
        (TASK9_ROUND_TRIP_STEP_NAME, 30, TASK9_ROUND_TRIP_SELECTOR),
        (TASK9_LARGE_STEP_NAME, 30, TASK9_LARGE_SELECTOR),
    ):
    ...
    require_task9_tokens(
        ...
        (
            ...
            '("reports.range.one_month", 2,',
            '("reports.range.three_months", 3,',
            '("reports.range.six_months", 4,',
            '("reports.range.one_year", 5,',
            '"81 kg"',
            ...
            '"1/2 uygun hedef günü"',
            '"hedefsiz 1 gün dışlandı"',
    ...
    require_task9_tokens(
        ...
        (
            ...
            'let segments = format.buttons.allElementsBoundByIndex',
            'XCTAssertEqual(segments.count, 3)',
            'XCTAssertTrue(segment.exists)',
            'XCTAssertTrue(segment.isHittable)',
            'XCTAssertFalse(segment.label.isEmpty)',
            'if width.isFinite, width > 0',
            'XCTAssertGreaterThanOrEqual(width + 0.01, 44)',
    ...
    scroll_helper = (
        root / "HealthTrackingAppUITests/M4UITestScrollHelper.swift"
    ).read_text(encoding="utf-8")
    require_task9_tokens(
        scroll_helper,
        (
    ...
    with tempfile.TemporaryDirectory(prefix="m4-task9-stage-a-verifier-") as directory:
        ...
        mutations = (
            ...
            (
                "HealthTrackingAppUITests/M4ReportsAcceptanceUITests.swift",
                '("reports.range.three_months", 3,',
                '("reports.range.three_months", 2,',
                "real-app acceptance contract",
            ),
            (
                "HealthTrackingAppUITests/M4ReportsAcceptanceUITests.swift",
                'XCTAssertEqual(initialValue, "0", "Photo binaries must remain opt-in by default.")',
                'XCTAssertEqual(initialValue, "1", "Photo binaries must remain opt-in by default.")',
                "real-app acceptance contract",
            ),
            (
                "HealthTrackingAppUITests/M4ReportsAcceptanceUITests.swift",
                '"1/2 uygun hedef günü"',
                '"2/2 uygun hedef günü"',
                "real-app acceptance contract",
            ),
            (
                "HealthTrackingAppUITests/M4ReportsAccessibilityUITests.swift",
            ...
            (
                "App/Application/TrackerFeatureBundle.swift",
                'bodyMetric("00000000-0000-4000-8000-00000000b407", "2026-06-10T09:00:00Z", .weight, 81.0, "kg")',
                'bodyMetric("00000000-0000-4000-8000-00000000b407", "2026-07-10T09:00:00Z", .weight, 81.0, "kg")',
                "real repository/export/share wiring contract",
            ),
            (
                "HealthTrackingAppUITests/M4ReportsAccessibilityUITests.swift",
...
TASK7_TEST_ASSET_SHA256 = {
    ...
    "Packages/HealthTrackingModules/Tests/ReportsKitTests/ReportExportCoordinatorTests.swift": (
        "603a0e3e583d35e68c1955fe4488f39612caae9cdd21f8b4f323668ce073ee35"
    ),
}
...
TASK7_PRODUCTION_ASSET_SHA256: dict[str, str] = {
    ...
    "Packages/HealthTrackingModules/Sources/ReportsKit/Export/DescriptorBoundFileSystem.swift": (
        "8928caf59132771b6bd731eda173b257f4cac22c445a1c92e32ba79a1265348a"
    ),
    "Packages/HealthTrackingModules/Sources/ReportsKit/Export/ReportExportCoordinator.swift": (
