...
final class BloodworkRepositoryTests: XCTestCase {
    ...
    func testCRUDTrimsValuesAndReturnsStableDateIDOrdering() async throws {
        ...
        let initialOrder = try await repository.fetchResults().map(\.id)
        XCTAssertEqual(initialOrder, [newer.id, older.id])
        ...
        let updatedOrder = try await repository.fetchResults().map(\.id)
        XCTAssertEqual(updatedOrder, [updated.id, newer.id])

        try await repository.deleteResult(
            id: updated.id,
            expectedUpdatedAt: updated.updatedAt
        )
        let finalOrder = try await repository.fetchResults().map(\.id)
        XCTAssertEqual(finalOrder, [newer.id])
    }
