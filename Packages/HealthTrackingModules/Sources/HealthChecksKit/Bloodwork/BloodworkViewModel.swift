...
public enum BloodworkEditFailure: Equatable, Sendable {
    case update(id: UUID)
    case delete(id: UUID)
}

private enum PendingBloodworkEditMutation: Sendable {
    case update(
        snapshot: BloodworkResultSnapshot,
        input: BloodworkResultInput
    )
    case delete(snapshot: BloodworkResultSnapshot)

    var failure: BloodworkEditFailure {
        switch self {
        case let .update(snapshot, _):
            .update(id: snapshot.id)
        case let .delete(snapshot):
...
public final class BloodworkViewModel {
    ...
    public private(set) var editFailure: BloodworkEditFailure?
    ...
    @ObservationIgnored
    private var pendingEditMutation: PendingBloodworkEditMutation?
    ...
    public func update(
        ...
        await beginEditMutation(.update(snapshot: snapshot, input: input))
    }

    @discardableResult
    public func delete(_ snapshot: BloodworkResultSnapshot) async -> Bool {
        await beginEditMutation(.delete(snapshot: snapshot))
    }

    @discardableResult
    public func retryEditMutation() async -> Bool {
        guard editPhase == .failed,
              let pendingEditMutation else {
            return false
        }
        return await performEditMutation(pendingEditMutation)
    }

    private func beginEditMutation(
        _ mutation: PendingBloodworkEditMutation
    ) async -> Bool {
        guard editPhase != .saving else { return false }
        pendingEditMutation = mutation
        return await performEditMutation(mutation)
    }

    private func performEditMutation(
        _ mutation: PendingBloodworkEditMutation
    ) async -> Bool {
        editGeneration &+= 1
        ...
        do {
            switch mutation {
            case let .update(snapshot, input):
    ...
    public func prepareForCreation() {
        guard !hasMutationInFlight, editPhase != .saving else { return }
