...
public struct BloodworkListView: View {
    ...
    private func detailMutationState(_ snapshot: BloodworkResultSnapshot) -> some View {
        if viewModel.editFailure == .delete(id: snapshot.id) {
            VStack(alignment: .leading, spacing: AppSpacing.small) {
                ...
                Button(localized("bloodwork.retry")) {
                    Task {
                        if await viewModel.retryEditMutation() {
                            onCommittedMutation()
                            destination = .list
                        }
                    }
    ...
    private var isEditorFailure: Bool {
        ...
        case .editor(nil):
            ...
            case .saveFailed:
                true
            case .idle, .saving, .saved, .undoFailed, .undoing:
                false
            }
        case let .editor(.some(id)):
            viewModel.editFailure == .update(id: id)
        case .list, .detail(_):
            false
        }
    }

    private var isEditorRetryLocked: Bool {
        isEditorFailure
    }
