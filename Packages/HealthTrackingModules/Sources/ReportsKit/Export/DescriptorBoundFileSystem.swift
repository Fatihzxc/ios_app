...
/// Active retries retain a verified parent descriptor. Terminal records retain
/// only identities/paths, are retried opportunistically, and consume capacity so
/// repeated failures cannot grow disk usage without bound.
final class ReportExportStageResidueRegistry: @unchecked Sendable {
    ...
    private struct Entry {
        ...
        let parentURL: URL
        let parentIdentity: ReportExportFileIdentity
        let fileIdentity: ReportExportFileIdentity
        var attempts: Int
    }
    ...
    var retainedCount: Int {
        lock.lock()
        defer { lock.unlock() }
        return entries.count + terminals.count
    }
    ...
    func reserve() throws -> UUID {
        ...
        lock.lock()
        defer { lock.unlock() }
        guard reservations.count + entries.count + terminals.count < maximumCount else {
            throw ReportExportError.temporaryStorageFailed
        }
        let id = UUID()
    ...
    func retain(
        ...
        parentURL: URL,
        fileIdentity: ReportExportFileIdentity
    ) {
        let parentIdentity: ReportExportFileIdentity
        do {
            parentIdentity = try ReportExportDescriptorIO.metadata(
                for: parent.rawValue
            ).identity
        } catch {
            release(id)
        ...
        lock.unlock()
        schedule(id, attempt: 0)
    }

    private func schedule(_ id: UUID, attempt: Int) {
    ...
    private func retryTerminals() {
        lock.lock()
        let pending = Array(terminals.values)
        lock.unlock()
        for terminal in pending {
            do {
                try ReportExportNamespaceAuthority.shared.transaction {
                    let parent = try ReportExportDescriptorIO.openDirectoryTree(
                        terminal.parentURL.standardizedFileURL
                    )
                    try Self.removeExactResidue(
                        parent: parent,
                        expectedParent: terminal.parentIdentity,
                        fileIdentity: terminal.fileIdentity
                    )
                }
                lock.lock()
                terminals.removeValue(forKey: terminal.id)
                lock.unlock()
            } catch {
                continue
            }
    ...
    private static func removeExactResidue(
        parent: ReportExportFileDescriptor,
        expectedParent: ReportExportFileIdentity,
        fileIdentity: ReportExportFileIdentity
    ) throws {
        guard try ReportExportDescriptorIO.metadata(for: parent.rawValue).identity
                == expectedParent else {
            throw ReportExportError.cleanupIdentityMismatch
        }
        let names = try ReportExportDescriptorIO.directoryNames(parent.rawValue)
        for name in names {
            let metadata: ReportExportDescriptorIO.Metadata
            do {
                metadata = try ReportExportDescriptorIO.metadata(
                    at: name,
            ...
            guard metadata.identity == fileIdentity else { continue }
            let opened = name.withCString {
                Darwin.openat(parent.rawValue, $0, O_RDONLY | O_NOFOLLOW | O_CLOEXEC)
            }
            guard opened >= 0 else { throw ReportExportDescriptorIO.currentPOSIXError() }
            let descriptor = ReportExportFileDescriptor(taking: opened)
            guard try ReportExportDescriptorIO.metadata(for: opened).identity == fileIdentity else {
                throw ReportExportError.cleanupIdentityMismatch
            }
            try ReportExportDescriptorIO.setNoUnlink(false, on: opened)
            let result = try ReportExportDescriptorIO.withRemovalAllowed(
                in: parent.rawValue,
                requirePrivateInvariant: false
            ) {
                name.withCString { Darwin.unlinkat(parent.rawValue, $0, 0) }
            }
            if result != 0 {
...
final class ReportExportVisibleDirectoryLease: @unchecked Sendable {
    ...
    private let identity: ReportExportFileIdentity
    private let originalFlags: UInt32
    private let lock = NSLock()
    private var active = true
    ...
    init(
        ...
        let established = try ReportExportNamespaceAuthority.shared.transaction {
            let current = try ReportExportDescriptorIO.metadata(for: descriptor.rawValue)
            guard current.isDirectory, current.owner == Darwin.geteuid() else {
                throw ReportExportError.unsafeTemporaryPath
            }
            do {
                ...
                return (current.identity, current.flags)
            } catch {
    ...
    func release() throws {
        ...
        try ReportExportNamespaceAuthority.shared.transaction {
            ...
            guard Darwin.fchflags(descriptor.rawValue, originalFlags) == 0 else {
                throw ReportExportDescriptorIO.currentPOSIXError()
            }
            let restored = try ReportExportDescriptorIO.metadata(for: descriptor.rawValue)
            guard restored.identity == identity, restored.flags == originalFlags else {
                throw ReportExportError.unsafeTemporaryPath
            }
        }
        active = false
        ...
        do {
            return try ReportExportNamespaceAuthority.shared.transaction {
                ...
                guard parentMetadata.isDirectory,
                      ...
                      (!privateParent || parentMetadata.hasNoUnlink),
                      privateParent
                        || (parentMetadata.flags & UInt32(UF_IMMUTABLE)) == 0 else {
                    throw ReportExportError.unsafeTemporaryPath
                }
                ...
                do {
                    ...
                    initial = try metadata(for: opened)
                } catch {
                    try? setFlags(
                        parentMetadata.flags,
                        on: parent,
                ...
                do {
                    guard Darwin.fchmod(opened, S_IRUSR | S_IWUSR) == 0,
                          initial.isRegularFile,
                          initial.linkCount == 1 else {
                        throw ReportExportError.unsafeTemporaryPath
                    }
                    try beforeUnlink()
                    ...
                    guard before.identity == initial.identity,
                          ...
                          visible.isRegularFile else {
                        throw ReportExportError.unsafeTemporaryPath
                    }
                    let unlinkFlags = privateParent
                        ? parentMetadata.flags & ~namespaceProtectionFlags
                        : parentMetadata.flags
                    try setFlags(
                        unlinkFlags,
                        on: parent,
