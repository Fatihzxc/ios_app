...
final class ReportExportLifetimeCleanupRegistry: @unchecked Sendable {
    ...
    private func retry(cleanupID: UUID, scheduleToken: UUID?) {
        ...
        do {
            ...
            var terminalDecisionToken: UUID?
            ...
            if var current = entries[cleanupID] {
                ...
                if current.attempts >= maximumRetryAttempts {
                    ...
                    if current.terminalRecovery != nil {
                        // Keep the bounded operation unscheduled. A later
                        // allocation calls `retryPendingNow`, allowing identity
                        // discovery or cleanup to recover without an infinite task.
                        current.attempts = maximumRetryAttempts
                        let decisionToken = UUID()
                        current.scheduledToken = decisionToken
                        terminalDecisionToken = decisionToken
            ...
            if let terminalRecovery, let terminalDecisionToken {
                let recovery = terminalRecovery()
                lock.lock()
                if var current = entries[cleanupID],
                   current.isRunning,
                   current.scheduledToken == terminalDecisionToken {
                    if let recovery {
                        entries.removeValue(forKey: cleanupID)
...
final class FileManagerReportExportTemporaryFileSystem:
    ...
    func writeAtomically(_ data: Data, to url: URL) throws {
        ...
        let staging = try createPrivateAnonymousStagingFile(
